...
class MultimodalRetrievalService:
    ...
    async def search_by_image(
        ...
        if modality_filter:
            results = [
                r for r in results
                if r.get("modality") == modality_filter
            ]
    ...
    async def search_hybrid(
        ...
        reranked_results = await self._rerank_multimodal(
            query_type="hybrid",
            query_data={"text": text, "image": image_data},
            results=results
        )
    ...
    async def _encode_image(self, image_data: bytes) -> list[float]:
        """
        编码图像为向量
    ...
    def _fuse_embeddings(
        ...
        # 加权平均
        fused = [
            t * text_weight + i * image_weight
            for t, i in zip(text_emb, image_emb, strict=False)
        ]
    ...
    async def _generate_image_caption(self, image_data: bytes) -> str:
        """
        生成图像描述
