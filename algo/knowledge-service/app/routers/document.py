...
class UploadDocumentRequest(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(default_factory=dict, description="元数据")
...
class ProcessDocumentRequest(BaseModel):
    ...
    document_id: str
    chunk_size: int | None = Field(500, description="分块大小")
    chunk_overlap: int | None = Field(50, description="分块重叠")
...
async def process_document(
    request: ProcessDocumentRequest, session: AsyncSession = Depends(get_session)
):
    """
    处理文档（提取内容并分块）
    ...
    try:
        ...
        # 下载文件
        file_data = await doc_manager.download_document(request.document_id, document.file_path)
        ...
        return ProcessDocumentResponse(
            document_id=request.document_id, chunks_count=len(chunks), status=document.status.value
        )
...
async def list_documents(
    ...
    try:
        ...
        documents, total = await repo.list_by_knowledge_base(
            knowledge_base_id=knowledge_base_id, tenant_id=x_tenant_id, offset=offset, limit=limit
        )
        ...
        return ListDocumentsResponse(documents=docs_response, total=total)
...
async def delete_document(
    document_id: str,
    _x_user_id: str = Header("anonymous", alias="X-User-ID"),
    session: AsyncSession = Depends(get_session),
):
    """
    删除文档
...
async def download_document(_document_id: str):
    """
    获取文档下载 URL
    ...
    try:
        ...
        # 初始化文档管理器
        minio = get_minio_client()
        get_document_manager(minio)

        # 生成下载 URL
        # url = await doc_manager.get_download_url(document.file_path)

        return {"download_url": "TODO", "expires_in": 3600}
