...
async def demo_resilience():
    ...
    from app.core.resilience import with_resilience, get_circuit_breaker
...
def demo_token_counter():
    ...
    from app.core.token_counter import TokenCounter, CostCalculator
...
def demo_budget_manager():
    ...
    for i in range(5):
        ...
        print(f"    第{i+1}次: +${usage:.2f} → ${status.current_usage_usd:.2f} / ${status.daily_limit_usd:.2f} ({status.usage_percent*100:.1f}%) {emoji}")
