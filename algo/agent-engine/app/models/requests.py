...
class ExecuteTaskRequest(BaseModel):
    ...
    tools: list[str] | None = Field(default=None, description="可用工具列表，None 表示使用所有工具")
    conversation_id: str | None = Field(
        default=None, max_length=100, description="会话 ID，用于记忆管理"
    )
    tenant_id: str | None = Field(default=None, max_length=100, description="租户 ID")
...
class RegisterToolRequest(BaseModel):
    ...
    function: str = Field(..., description="函数路径，格式：module.path:function_name")
    parameters: dict[str, Any] = Field(
        ..., description="OpenAI Function JSON Schema 格式的参数定义"
    )
    init_args: list[Any] = Field(default_factory=list, description="初始化位置参数")
    init_kwargs: dict[str, Any] = Field(default_factory=dict, description="初始化关键字参数")
    category: str | None = Field(default="custom", description="工具分类")

    @field_validator("function")
    @classmethod
    def validate_function_path(cls, value: str) -> str:
