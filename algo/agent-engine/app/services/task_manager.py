    ...
    def _deserialize_result(self, data_str: str) -> AgentResult:
        ...
        for step_data in data.get("steps", []):
            step = AgentStep(
                ...
                step_type=StepType(step_data["step_type"]) if isinstance(step_data["step_type"], str) else step_data["step_type"],
                content=step_data["content"],
                tool_name=step_data.get("tool_name"),
                tool_input=step_data.get("tool_input"),
                tool_output=step_data.get("tool_output"),
                timestamp=datetime.fromisoformat(step_data["timestamp"]) if step_data.get("timestamp") else datetime.utcnow(),
            )
            steps.append(step)
