...
class CheckpointManager:
    ...
    def __init__(self, redis_client: Any | None = None):
        ...
        logger.info(
            ...
            f"(storage={'redis' if redis_client else 'local'})"
        )
...
class LangGraphWorkflowEngine:
    ...
    def create_workflow(self, config: dict) -> str:
        ...
        logger.info(
            ...
            f"({len(nodes)} nodes, entry={config['entry']})"
        )
        ...
        while state.current_node:
            ...
            try:
                if node.node_type == NodeType.ACTION:
                    ...
                    # 决策节点
                    if node.condition:
                        decision = await self._execute_function(
                            node.condition,
                            state.variables
                        )
    ...
    def _get_next_node(self, node: WorkflowNode, context: dict) -> str | None:
        ...
        if "conditional_next" in node.metadata:
            ...
            for condition_key, next_node_id in conditional_next.items():
                if condition_key == "else" and not decision:
                    return next_node_id
                elif decision and condition_key != "else":
                    return next_node_id
