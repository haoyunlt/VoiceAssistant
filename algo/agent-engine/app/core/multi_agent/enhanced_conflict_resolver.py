...
class EnhancedConflictResolver:
    ...
    async def resolve_conflict(
        self, agent1: Agent, agent2: Agent, conflict_type: ConflictType, context: dict[str, Any]
    ) -> Resolution:
        """
        解决冲突
    ...
    async def _resolve_by_priority(self, agent1: Agent, agent2: Agent, context: dict) -> Resolution:
        """基于优先级解决资源竞争"""
        # 1. 获取agent优先级
    ...
    async def _resolve_by_voting(self, agent1: Agent, agent2: Agent, context: dict) -> Resolution:
        """通过投票解决意见分歧"""
        # 1. 收集意见
...
格式: agent1/agent2: 理由"""
        ...
        # 3. 解析投票结果
        winner = agent1 if "agent1" in response.lower()[:10] else agent2
