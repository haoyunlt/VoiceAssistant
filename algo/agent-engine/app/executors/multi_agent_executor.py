...
class MultiAgentExecutor:
    ...
    async def execute_hierarchical(
        ...
        Returns:
            执行结果
        """
        logger.info(f"Starting hierarchical multi-agent execution")
...
Subtask:"""
    ...
    async def _synthesize_results(self, task: str, results: List[Dict]) -> str:
        """综合多个Agent的结果"""
        results_text = "\n\n".join([
...
Provide a comprehensive final answer:"""
    ...
    async def _check_consensus(self, opinions: List[Dict]) -> Dict:
        """检查是否达成共识"""
        opinions_text = "\n".join([
...
"""
    ...
    async def _vote_conclusion(self, task: str, discussion_history: List[Dict]) -> str:
        """通过投票得出结论"""
        all_opinions = []
        for round_data in discussion_history:
