...
class OCRRequest(BaseModel):
    ...
    image_base64: Optional[str] = Field(None, description="图像 Base64 编码")
    languages: Optional[List[str]] = Field(None, description="语言列表（如 ['ch', 'en']）")
...
class ImageAnalysisResponse(BaseModel):
    ...
    objects: Optional[List[Dict[str, Any]]] = Field(None, description="检测到的物体")
    scene: Optional[str] = Field(None, description="场景类型")
    colors: Optional[List[str]] = Field(None, description="主要颜色")
    text: Optional[str] = Field(None, description="图像中的文本（OCR）")
